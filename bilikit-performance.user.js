...
`;
  ...
  function init$5(cfg) {
    ...
    const rewriteJsonText = (raw, source) => {
      if (typeof raw !== "string" || !CDN_BODY_SIGNAL_RE.test(raw)) return null;
      try {
        const obj = parseJson(raw);
        if (!rewritePlayurl$1(obj, source)) return null;
      ...
      class X extends OX {
        ...
        get response() {
          ...
          if (r && typeof r === "object") {
            try {
              rewritePlayurl$1(r, `xhr:${this.__cdnUrl}`);
            } catch (_) {
            }
          }
          return r;
        ...
        __cdnText(raw) {
          ...
          return rewriteJsonText(raw, `xhr:${this.__cdnUrl}`) || raw;
        }
      }
